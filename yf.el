...
(defmacro yf-def-binop (name op &optional transform)
  "Define a binary money operation NAME using arithmetic operator OP."
  `(defun ,name (a b)
     (yf-expect-money (list a b))
     (let ((n1 ,(if transform `(,transform (car a)) `(car a)))
           (n2 (car b)))
...
(defun yf-define-built-ins ()
  (yf-def "+" (yf-push (yf-add (yf-pop) (yf-pop))))
  (yf-def "*" (yf-push (yf-mul (yf-pop) (yf-pop))))
  ...
  (yf-def "PRINC" (princ (yf-to-string (yf-pop))))
